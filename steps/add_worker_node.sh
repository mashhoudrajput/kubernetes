...
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 <path_of_folder> <new_worker_node_hostname> <new_worker_node_ip> <existing_master_node_ip>"
    exit 1
fi
...
if ssh $SSH_USER@$NEW_WORKER_NODE_IP "kubectl get nodes | grep -q $NEW_WORKER_NODE_HOSTNAME"; then
    echo "Node $NEW_WORKER_NODE_HOSTNAME is already part of the cluster. Exiting."
    exit 1
fi
...
./join_worker.sh $NEW_WORKER_NODE_IP $EXISTING_MASTER_NODE_IP
